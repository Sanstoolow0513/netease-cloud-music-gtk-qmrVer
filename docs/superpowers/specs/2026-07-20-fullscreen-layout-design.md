...
> **历史规格（2026-07-20）**，非现役合同。发现/我的页页面级 1280 clamp 大体仍有效；**榜单右侧歌曲列表**已于 2026-07-24 改为 `SongListView` 宽屏双列铺满（去掉列表外层 1280 clamp）。现役说明见 `AGENTS.md`（歌曲列表条）与 `docs/ui-redesign-2026-07.md` §2.5。

日期：2026-07-20
状态：已获用户确认，待写实现计划（榜单列表限宽结论已被后续实现取代）
