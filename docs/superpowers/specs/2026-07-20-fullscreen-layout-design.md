...
| Banner 显示 | **B. 按图片比例自适应高度，零裁切** | A. 固定 320 铺满（仍裁切）；C. 原图居中+模糊背景（成本高） |
...
- **Banner**：去掉 `GtkOverlay` 写死的 `height-request: 320`。轮播图按图片比例自适应高度：图片加载完成（拿到 texture 宽高）且栏宽确定后，轮播容器高度 = `栏宽 × 图高 / 图宽`，钳制在 **260~380px** 之间防止极端比例；切换轮播页时按当前页图片重算，轻微高度跳动可接受。
